...
echo "$CHANGED" | while read -r STATUS FILE1 FILE2; do
  ...
  # 복사 제외 대상: 보안/문서성 파일
  if [[ "$FILE1" == ".env" || "$FILE1" == .envs/* || "$FILE1" == "README.md" ]]; then
    echo "🚫 $FILE1 → 제외 대상 (복사하지 않음)"
    continue
  fi
  ...
  case "$STATUS" in
    ...
    A|M)
      if [ -f "$FILE1" ]; then
        echo "업로드 파일: $FILE1"
        echo "컨테이너 경로: $TARGET_ROOT/$FILE1"
        ...
        # 컨테이너 내 대상 경로가 없을 수도 있으니 미리 만들어줌
